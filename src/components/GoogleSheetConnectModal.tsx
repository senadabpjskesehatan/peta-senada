...
import { 
  ...
  Copy, Sparkles, Globe, ShieldCheck, Check, FileCheck, Layers, Database, Clock
} from 'lucide-react';
import { SyncConfig, ColumnDef, SheetRow } from '../types';
import { parseExcelBufferToRows, parseCSVToRows } from '../utils/dataProcessor';
import { getUploadedDatasetsFromFirebase, UploadedDatasetRecord } from '../utils/firebaseStorage';
...
export const GoogleSheetConnectModal: React.FC<GoogleSheetConnectModalProps> = ({
  ...
  const [tab, setTab] = useState<'url' | 'file' | 'paste' | 'history'>('file');
  ...
  const [firestoreHistory, setFirestoreHistory] = useState<UploadedDatasetRecord[]>([]);
  const [isLoadingHistory, setIsLoadingHistory] = useState(false);

  useEffect(() => {
    if (tab === 'history') {
      setIsLoadingHistory(true);
      getUploadedDatasetsFromFirebase(20)
        .then((records) => {
          setFirestoreHistory(records);
        })
        .catch(() => {})
      ...
      <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-2xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden my-auto animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex border-b border-slate-100 px-6 pt-2 bg-slate-50/40">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-bold border-b-2 transition-all ${
              tab === 'history' ? 'border-amber-600 text-amber-600' : 'border-transparent text-slate-400 hover:text-slate-700'
            }`}
          >
            <Database className="w-3.5 h-3.5" />
            <span>Riwayat Firestore</span>
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 text-xs text-slate-700 space-y-4">
          ...
          {/* TAB 4: FIRESTORE UPLOAD HISTORY */}
          {tab === 'history' && (
            <div className="space-y-4">
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-2xl flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h4 className="font-extrabold text-amber-900 text-xs">Pencatatan Dataset Firestore</h4>
                    <p className="text-[11px] text-amber-700">Setiap data yang diunggah tersimpan secara permanen di cloud Firestore.</p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => {
                    setIsLoadingHistory(true);
                    getUploadedDatasetsFromFirebase(20)
                      .then((rec) => setFirestoreHistory(rec))
                      .finally(() => setIsLoadingHistory(false));
                  }}
                  className="p-1.5 bg-white hover:bg-amber-100 border border-amber-300 rounded-xl text-amber-800 transition-colors"
              ...
              {isLoadingHistory ? (
                <div className="py-12 text-center text-slate-400 space-y-2">
                  ...
                  <p className="font-medium text-xs">Memuat catatan dataset dari Firestore...</p>
                </div>
                ...
                <div className="py-12 text-center text-slate-400 border border-dashed border-slate-200 rounded-2xl">
                  ...
                  <p className="font-bold text-slate-700 text-xs">Belum Ada Riwayat Upload di Firestore</p>
                  <p className="text-[11px] text-slate-500 mt-1">Unggah file Excel, CSV, atau tautan Google Sheet untuk mencatat dataset di Firestore.</p>
                </div>
              ) : (
                <div className="space-y-2.5 max-h-[380px] overflow-y-auto pr-1">
                  {firestoreHistory.map((item) => (
                    <div key={item.id} className="p-3.5 bg-slate-50 border border-slate-200 rounded-2xl hover:border-amber-300 hover:bg-amber-50/30 transition-all flex items-center justify-between gap-3">
                      <div className="space-y-1 min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="font-extrabold text-slate-900 text-xs truncate">{item.fileName || 'Dataset Terunggah'}</span>
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold capitalize shrink-0 ${
                            item.sourceType === 'excel' ? 'bg-emerald-100 text-emerald-800' :
                            item.sourceType === 'csv' ? 'bg-cyan-100 text-cyan-800' : 'bg-blue-100 text-blue-800'
                          }`}>
                            {item.sourceType}
                          </span>
                        </div>
                        <div className="flex items-center gap-3 text-[11px] text-slate-500 font-medium">
                          <span className="flex items-center gap-1">
                            ...
                            {new Date(item.uploadedAt).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' })}
                          </span>
                          <span>•</span>
                          <span>{item.rowCount} baris data</span>
                          <span>•</span>
                          <span>{item.columnCount} kolom</span>
                        </div>
                      </div>
                        ...
                        <button
                          ...
                          onClick={() => {
                            onImportData(
                              ...
                              item.fileName,
                              targetDestination === 'new_page',
                              item.sourceType === 'url' ? 'csv' : item.sourceType as any,
                              importMode
                            );
                            onClose();
                          }}
                          className="px-3 py-1.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-bold text-[11px] shadow-xs shrink-0 transition-all"
                        >
                          Muat Data Ini
                        </button>
                      )}
